...
export async function GET(req) {
  ...
  const [rows] = await conn.execute("SELECT id, username, nama, nomorhp FROM tb_praktikan WHERE id=?", [id]);
  await conn.end();
...
export async function PUT(req) {
  ...
  try {
    // Ambil data lama
    const [rows] = await conn.execute("SELECT * FROM tb_praktikan WHERE id=?", [id]);
    if (rows.length === 0) {
    ...
    if (hashedPassword) {
      await conn.execute(
        "UPDATE tb_praktikan SET username=?, nama=?, nomorhp=?, password=? WHERE id=?",
        [newUsername, newNama, newNomorhp, hashedPassword, id]
      );
    } else {
      await conn.execute(
        "UPDATE tb_praktikan SET username=?, nama=?, nomorhp=? WHERE id=?",
